...
namespace refresh_item_data {
    void function(const event::EventContext& ctx)
    {
        ...
        game_update_packet.dec_item_data_size = static_cast<uint32_t>(ctx.item_db->get_size());
        game_update_packet.data_size = static_cast<uint32_t>(ctx.item_db->get_compressed_size());

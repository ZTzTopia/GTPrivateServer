...
namespace server {
    ...
    public:
        ...
        char *get_struct(ENetPacket *packet, int length) {
        ...
        GameUpdatePacket *get_struct(ENetPacket *packet) {
            if (packet->dataLength >= sizeof(GameUpdatePacket)) {
                auto *gameUpdatePacket = (GameUpdatePacket*)(packet->data + 4);
                if ((gameUpdatePacket->unk6 & 8) == 0) {
                    gameUpdatePacket->data_extended_length = 0;
                }
                else {
                    if (packet->dataLength < gameUpdatePacket->data_extended_length + sizeof(GameUpdatePacket)) {
                        return nullptr;
                    }
                }
        ...
        uint8_t *get_extended_data(GameUpdatePacket *gameUpdatePacket) {
            if ((gameUpdatePacket->unk6 & 8) != 0) {
                return reinterpret_cast<uint8_t*>(&gameUpdatePacket->data_extended);
            }

            return nullptr;
        }

        int send_packet(ENetPeer *peer, eNetMessageType type, const std::string &data) {
            int ret = -1;
            if (peer) {
                ENetPacket *packet = enet_packet_create(nullptr, data.length() + 5, 1);
                *(eNetMessageType *)packet->data = type;
                std::memcpy(packet->data + 4, data.data(), data.length());

                ret = enet_peer_send(peer, 0, packet) != 0;
                if (ret) {
